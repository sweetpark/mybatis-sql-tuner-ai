...
import io.github.sweetpark.sqlanalyzer.intellij.service.AiChatClient;
import io.github.sweetpark.sqlanalyzer.intellij.service.SqlAnalyzerService;
...
public class SqlAnalyzerPanel extends JPanel {
	...
	private static final Logger log = LoggerFactory.getLogger(SqlAnalyzerPanel.class);
	...
	private void runAiAnalysis() {
		...
		SqlAnalyzerConfig dbConfig = DbSettingsDialog.loadConfig(project);
		...
		ProgressManager.getInstance().run(new Task.Backgroundable(project, "AI 분석 중...") {
			...
			public void run(@NotNull ProgressIndicator indicator) {
				...
				try {
					...
					aiChatClient.streamChat(aiConfig, prompt, SqlAnalyzerPanel.this::appendDelta);
				} catch (Exception e) {
					error = e;
				}
			}
