...
class DbSettingsDialogTest {
	...
	void constants_check() {
		assertEquals("sql-tuner.jdbc.url", DbSettingsDialog.KEY_JDBC_URL);
		assertEquals("sql-tuner.jdbc.user", DbSettingsDialog.KEY_JDBC_USER);
		assertEquals("sql-tuner.jdbc.password", DbSettingsDialog.KEY_JDBC_PASSWORD);
	}
	...
	void loadConfig_test() {
		...
		SqlTunerConfig config = DbSettingsDialog.loadConfig(mockProject);
